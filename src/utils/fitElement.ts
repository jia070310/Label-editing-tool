...
import { fitTableToRect, updateAllCells } from './table'
...
export function fitNewElementToLabel(
  ...
  if (el.type === 'table') {
    const tw = round1(Math.min(availW, Math.max(8, 50 * scale)))
    const th = round1(Math.min(availH, Math.max(8, 32 * scale)))
    let table = fitTableToRect(el as TableElement, margin, margin, tw, th)
